...
donations_train <- donations_train[,-c(315:383, 388, 391, 419)] # remove RFA columns, all date columns
...
###### R2 = 0.8 ######
dt.mm <- model.matrix(~., data = donations_train[, -c(350)]) # remove TARGET_B columns, keep only variables
...
dt.mm_norm = scale(dt.mm, center=FALSE, scale=colSums(dt.mm))
dt.mm_df = data.frame(dt.mm_norm)
...
# logreg_df <- logreg_df[, -c(7)] # remove last date
TARGET_B <- train['TARGET_B']
logreg_df <- cbind(logreg_df[, !(names(logreg_df) == 'TARGET_D')], TARGET_B) 
logreg <- lm(TARGET_B~ . -1, data=logreg_df, family='binomial')
summary(logreg)
```
